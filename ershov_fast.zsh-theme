...
typeset -g _LAST_DIR=""
typeset -g _CACHED_PATH_COLOR=""
typeset -g _GIT_REPO_CHECK_CACHE=""
typeset -g _GIT_STATUS_CACHE=""
typeset -g _CACHE_TIMESTAMP=0
...
# =============================================================================
# УДАЛЕНИЕ ПО РАЗДЕЛИТЕЛЯМ С ОПТИМИЗАЦИЕЙ
# =============================================================================
...
backward-kill-path-component() {
  ...
  BUFFER="${buf[1,$((pos))]}${buf[$((cur+1)),-1]}"
  CURSOR=$pos
  zle reset-prompt
}

# Удаление пути ВПЕРЕД (до следующего разделителя)
forward-kill-path-component() {
  local buf="$BUFFER"
  local len=${#buf}
  local cur="$CURSOR"
  local start_pos=$cur
  ...
  # Идём вперёд, пока не найдём разделитель или конец строки
  while [[ $pos -lt $len && ${buf[$pos]} != "[" && ${buf[$pos]} != "@" && \
           ${buf[$pos]} != ":" && ${buf[$pos]} != "/" && \
           ${buf[$pos]} != "." && ${buf[$pos]} != "-" ]]; do
    (( pos++ ))
  done

  # Если нашли разделитель, включаем его в удаление
  if [[ $pos -lt $len && ${buf[$pos]} =~ [@:/\.-] ]]; then
    (( pos++ ))
  fi

  # Удаляем от текущей позиции до найденной позиции
  if [[ $pos -gt $start_pos ]]; then
    BUFFER="${buf[1,$start_pos]}${buf[$((pos+1)),-1]}"
    CURSOR=$start_pos
  fi
...
# Умное удаление ВСЕГО слова (с учётом путей)
backward-kill-smart-word() {
  local buf="$BUFFER"
  local cur="$CURSOR"
  local context_chars=${buf[1,$cur]}

  # Определяем контекст: если перед курсором путь, используем path component удаление
  if [[ $context_chars =~ [/@:\.-] ]]; then
    backward-kill-path-component
  else
...
zle -N forward-kill-path-component
zle -N backward-kill-smart-word

# =============================================================================
# ОПТИМИЗИРОВАННАЯ ФУНКЦИЯ ЦВЕТА ПУТИ С КЭШИРОВАНИЕМ
# =============================================================================
...
get_path_color() {
  ...
  # Используем кэш если директория не изменилась и кэш свежий (5 секунд)
  if [[ "$current_dir" == "$_LAST_DIR" ]] && \
     (( current_time - _CACHE_TIMESTAMP < 5 )); then
    PATH_COLOR="$_CACHED_PATH_COLOR"
    return
  fi
  ...
  local perms
  # Используем более быстрый метод получения прав
  if command -v stat >/dev/null 2>&1; then
    perms=$(stat -c "%a" "$current_dir" 2>/dev/null) || {
      PATH_COLOR="%F{red}"
      _CACHED_PATH_COLOR="$PATH_COLOR"
      return
    }
  else
    # Fallback для систем без stat
    PATH_COLOR="%F{blue}"
    _CACHED_PATH_COLOR="$PATH_COLOR"
    return
  fi
...
# =============================================================================
# ОПТИМИЗИРОВАННАЯ ФУНКЦИЯ GIT ПРОМПТА С КЭШИРОВАНИЕМ
# =============================================================================

git_prompt_info() {
  local current_time=$(date +%s)
  local cache_ttl=2  # Кэшируем на 2 секунды
  
  # Быстрая проверка: если не в git репозитории, сразу выходим
  if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    GIT_PROMPT=""
    _GIT_REPO_CHECK_CACHE=""
    return
  fi
  
  local repo_root=$(git rev-parse --show-toplevel 2>/dev/null)
  
  # Используем кэш, если репозиторий не изменился и кэш свежий
  if [[ "$repo_root" == "$_GIT_REPO_CHECK_CACHE" ]] && \
     (( current_time - _CACHE_TIMESTAMP < cache_ttl )); then
    GIT_PROMPT="$_GIT_STATUS_CACHE"
    return
  fi
  
  _GIT_REPO_CHECK_CACHE="$repo_root"
  _CACHE_TIMESTAMP=$current_time
  ...
  for line in $status_lines; do
    ...
    # Прерываем если нашли все типы статусов (оптимизация)
...
# Автоматически сбрасываем кэш при смене директории
chpwd() {
  zsh-cache-clear >/dev/null 2>&1
}

# Алиас для ручной очистки кэша
alias zsh-cache-clear='zsh-cache-clear'
...
# Умное удаление путей
bindkey '\e^?' backward-kill-path-component       # Alt + Backspace — удаление по разделителям (@, :, /, ., -)
bindkey '^[[3;3~' forward-kill-path-component     # Alt + Delete — удалить компонент пути вперёд
bindkey '^W' backward-kill-smart-word             # Ctrl + W — умное удаление слова (автоматически выбирает тип)
...
# =============================================================================
# ОБНОВЛЁННАЯ ФУНКЦИЯ PRECMD С КЭШИРОВАНИЕМ
# =============================================================================

new_line_before_prompt=yes
precmd() {
    # Обновляем Git информацию и цвет пути (с кэшированием)
    git_prompt_info
    get_path_color
...
# =============================================================================
# АЛИАСЫ И ДОПОЛНИТЕЛЬНЫЕ НАСТРОЙКИ
# =============================================================================
